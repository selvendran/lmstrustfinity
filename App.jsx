...
const { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, PieChart, Pie, Cell, LineChart, Line, ResponsiveContainer, Legend } = Recharts;
...
const DEMO_USERS = [
  { id:1, name:"Akash Kumar", role:"employee", dept:"Engineering" },
  { id:2, name:"Neha Sharma", role:"manager", dept:"Engineering" },
  { id:3, name:"Rajesh Mehta", role:"hr", dept:"HR" },
  { id:4, name:"Admin User", role:"admin", dept:"IT" },
  { id:5, name:"Arvind Rao", role:"reports", dept:"Finance" },
];

const ROLE_COLORS = { employee:"#2563eb", manager:"#16a34a", hr:"#d97706", admin:"#7c3aed", reports:"#dc2626" };
const ROLE_DESC = {
...
const TEAM_MEMBERS = [
  {id:1,name:"Akash Kumar",dept:"Engineering"},{id:3,name:"Priya M.",dept:"Sales"},
  {id:4,name:"Rahul S.",dept:"Engineering"},{id:5,name:"Anil K.",dept:"HR"},
  {id:6,name:"Ravi K.",dept:"Finance"},{id:7,name:"Amit D.",dept:"Engineering"},
];
...
const BALANCES = { 1:{earned:6,sick:4,menstrual:7,maternity:0,paternity:3,bereavement:2,lwp:0} };
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(15,23,42,0.55)",zIndex:900,display:"flex",alignItems:"center",justifyContent:"center",padding:16}}>
      <div style={{background:"#fff",borderRadius:16,padding:24,width:"100%",maxWidth:480,boxShadow:"0 24px 64px rgba(0,0,0,0.25)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          ...
          <button onClick={onClose} style={{border:"none",background:"#f1f5f9",borderRadius:"50%",width:32,height:32,cursor:"pointer",fontSize:18,color:"#64748b"}}>×</button>
        </div>
        {children}
      </div>
    </div>
...
function Calendar({year,month,setYear,setMonth,holidays,getLeaves}) {
  ...
  const hDates = new Set(Object.keys(hMap).map(Number));
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
        ...
        {Array.from({length:daysInMonth}).map((_,i)=>{
          ...
          return <div key={day} title={isHol?hMap[day]?.name:hasLeave?leaves.map(l=>l.empName||"Leave").join(", "):""} style={{textAlign:"center",padding:"5px 2px",fontSize:13,borderRadius:6,background:bg,color,fontWeight:fw,border:"1px solid #f1f5f9",minHeight:28,display:"flex",alignItems:"center",justifyContent:"center"}}>{day}</div>;
        })}
      </div>
      {Object.keys(hMap).length>0&&(
        <div style={{marginTop:10,display:"flex",flexWrap:"wrap",gap:6}}>
          {Object.values(hMap).map((h,i)=><span key={i} style={{fontSize:11,padding:"2px 8px",borderRadius:999,background:h.tentative?"#fef9c3":"#fee2e2",color:h.tentative?"#92400e":"#dc2626",fontWeight:700}}>{h.tentative?"🟡":"🔴"} {h.name}{h.tentative?" (Tentative)":""}</span>)}
        </div>
      )}
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(145deg,#0f2c5c 0%,#1e4d99 50%,#2563eb 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      <div style={{width:"100%",maxWidth:520}}>
        ...
        <div style={{background:"#fff",borderRadius:20,padding:32,boxShadow:"0 32px 80px rgba(0,0,0,0.3)"}}>
          ...
          <p style={{fontSize:13,color:"#64748b",textAlign:"center",margin:"0 0 24px"}}>Choose any role to explore the system</p>
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:24}}>
            {DEMO_USERS.map(u=>(
              <button key={u.id} onClick={()=>setSelected(u)} style={{padding:"14px 16px",border:selected?.id===u.id?`2px solid ${ROLE_COLORS[u.role]}`:"2px solid #e2e8f0",borderRadius:12,background:selected?.id===u.id?`${ROLE_COLORS[u.role]}08`:"#fff",cursor:"pointer",display:"flex",alignItems:"center",gap:14,textAlign:"left"}}>
                ...
                <span style={{fontSize:11,fontWeight:800,padding:"3px 10px",borderRadius:999,background:`${ROLE_COLORS[u.role]}15`,color:ROLE_COLORS[u.role],textTransform:"uppercase",flexShrink:0}}>{u.role}</span>
              </button>
            ))}
          </div>
          <button onClick={()=>selected&&onLogin(selected)} disabled={!selected} style={{width:"100%",padding:14,background:selected?"#2563eb":"#94a3b8",color:"#fff",border:"none",borderRadius:12,fontWeight:800,fontSize:16,cursor:selected?"pointer":"not-allowed",fontFamily:"inherit"}}>
            ...
            {myRequests.slice().reverse().slice(0,3).map(r=>(
              <div key={r.id} style={{borderLeft:`3px solid ${LEAVE_TYPES.find(l=>l.id===r.type)?.color||"#64748b"}`,paddingLeft:10}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:6}}>
                  <div><p style={{fontSize:12,fontWeight:700,color:"#0f172a",margin:"0 0 3px"}}>{fmtDate(r.start)} - {fmtDate(r.end)}</p><LeaveTypeBadge type={r.type}/></div>
  ...
  if(tab==="my-team") return (
    <div style={S.card}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:14}}>
        {TEAM_MEMBERS.map(m=>(
          <div key={m.id} style={{padding:16,border:"1px solid #e2e8f0",borderRadius:12,textAlign:"center"}}>
...
function ManagerView({tab,requests,onUpdate,holidays,calYear,calMonth,setCalYear,setCalMonth,getLeaves}) {
  ...
  if(tab==="dashboard") return (
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14,marginBottom:18}}>
        {stats.map(s=><div key={s.l} style={{...S.card,textAlign:"center",borderTop:`3px solid ${s.c}`}}><p style={{fontSize:28,fontWeight:900,color:s.c,margin:0}}>{s.v}</p><p style={{fontSize:12,color:"#64748b",margin:"4px 0 0"}}>{s.l}</p></div>)}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1.6fr 1fr",gap:14}}>
        <div style={S.card}>
          <h3 style={{fontSize:14,fontWeight:800,color:"#0f172a",margin:"0 0 14px"}}>Pending Requests</h3>
            ...
            {pending.map(r=>(
              <div key={r.id} style={{padding:12,border:"1px solid #e2e8f0",borderRadius:10,background:"#fafafa"}}>
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
                  ...
                  <div><p style={{fontSize:13,fontWeight:800,color:"#0f172a",margin:0}}>{r.empName}</p><p style={{fontSize:11,color:"#64748b",margin:"2px 0 0"}}>{fmtDate(r.start)} - {fmtDate(r.end)} ({r.days} days)</p></div>
                </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={S.card}>
            <h3 style={{fontSize:14,fontWeight:800,color:"#0f172a",margin:"0 0 12px"}}>Alerts</h3>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {pending.filter(r=>r.days>=5).map(r=>(
                <div key={r.id} style={{padding:10,background:"#fff7ed",border:"1px solid #fed7aa",borderRadius:8}}>
                  ...
                  <p style={{fontSize:11,color:"#374151",margin:0}}>{r.empName} – {r.days} Days</p>
                </div>
              ))}
              ...
              {pending.length>=2&&<div style={{padding:10,background:"#fef9c3",border:"1px solid #fde68a",borderRadius:8}}>
                <p style={{fontSize:12,fontWeight:800,color:"#92400e",margin:"0 0 3px"}}>🔄 Overlapping Leaves</p>
                <p style={{fontSize:11,color:"#374151",margin:0}}>Multiple members on same dates</p>
              </div>}
            </div>
          </div>
          <div style={S.card}>
            <h3 style={{fontSize:14,fontWeight:800,color:"#0f172a",margin:"0 0 12px"}}>Statistics</h3>
            {[["Total",requests.length],["Approved",approved.length],["Pending",pending.length]].map(([l,v])=>(
          ...
          <tbody>
            {requests.map(r=>(
              <tr key={r.id}>
                ...
                <td style={S.td}>{r.days}</td>
                <td style={S.td}><StatusBadge status={r.status}/></td>
                <td style={S.td}>{r.status==="pending"&&<div style={{display:"flex",gap:6}}><button onClick={()=>onUpdate(r.id,"approved")} style={{...S.btnGreen,padding:"4px 10px",fontSize:12}}>Approve</button><button onClick={()=>onUpdate(r.id,"rejected")} style={{...S.btnRed,padding:"4px 10px",fontSize:12}}>Reject</button></div>}</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  if(tab==="reports") {
    const deptData=["Engineering","Sales","HR","Finance"].map(dept=>({dept,earned:requests.filter(r=>r.dept===dept&&r.type==="earned").length,sick:requests.filter(r=>r.dept===dept&&r.type==="sick").length,other:requests.filter(r=>r.dept===dept&&!["earned","sick"].includes(r.type)).length}));
    return (
      <div style={S.card}>
...
function HRView({tab,requests,holidays,onAddHoliday,onDeleteHoliday}) {
  ...
  if(tab==="dashboard") return (
    <div style={{display:"grid",gridTemplateColumns:"3fr 2fr",gap:16}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            <div>
              ...
              <ResponsiveContainer width="100%" height={140}>
                <PieChart><Pie data={ltData} cx="50%" cy="50%" innerRadius={38} outerRadius={62} dataKey="value">{ltData.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip formatter={v=>`${v}%`}/></PieChart>
              </ResponsiveContainer>
            </div>
            <div style={{display:"flex",flexDirection:"column",justifyContent:"center",gap:8}}>
              <p style={{fontSize:12,fontWeight:700,color:"#64748b",margin:0}}>Absenteeism Rate</p>
              <div style={{textAlign:"center"}}><p style={{fontSize:28,fontWeight:900,color:"#dc2626",margin:0}}>8.2%</p><p style={{fontSize:11,color:"#64748b",margin:"2px 0 0"}}>This Month</p></div>
              <div style={{textAlign:"center"}}><p style={{fontSize:20,fontWeight:700,color:"#d97706",margin:0}}>4.5%</p><p style={{fontSize:11,color:"#64748b",margin:"2px 0 0"}}>Last Month</p></div>
            </div>
          </div>
        </div>
        <div style={S.card}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        <div style={S.card}>
          <h3 style={{fontSize:14,fontWeight:800,color:"#0f172a",margin:"0 0 12px"}}>Karnataka Holidays 2026</h3>
          <div style={{display:"flex",flexDirection:"column",gap:6}}>
            {holidays.map((h,i)=>(
              <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"7px 10px",borderRadius:8,background:"#f8fafc"}}>
                <div><span style={{fontSize:13,fontWeight:700,color:"#0f172a"}}>{h.name}</span>{h.tentative&&<span style={{fontSize:10,color:"#d97706",marginLeft:4}}>(Tentative)</span>}</div>
                <div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:12,color:"#64748b"}}>{fmtDate(h.date)}</span><button onClick={()=>onDeleteHoliday(h.id)} style={{border:"none",background:"none",color:"#dc2626",cursor:"pointer",fontSize:14}}>🗑</button></div>
              </div>
            ))}
          </div>
        ...
        <tbody>
          {holidays.map((h,i)=>(
            <tr key={i}>
              ...
              <td style={S.td}><span style={{fontSize:12,padding:"2px 10px",borderRadius:999,background:h.type==="national"?"#dbeafe":"#d1fae5",color:h.type==="national"?"#1d4ed8":"#065f46",fontWeight:700}}>{h.type}</span></td>
      ...
      <table style={{width:"100%",borderCollapse:"collapse"}}>
        ...
        <tbody>
          {[["Earned Leave","18 Days/Year","Up to 45 Days","No"],["Sick Leave","12 Days/Year","No","No"],["Menstrual Leave","12 Days/Year","No","No"],["Maternity Leave","26 Weeks","No","No"],["Paternity Leave","5 Days/Child","No","No"],["Bereavement Leave","3 Days","No","No"],["Leave Without Pay","Max 90 Days","No","No"]].map(([n,...v])=>(
            <tr key={n}><td style={{...S.td,fontWeight:700,color:"#0f172a"}}>{n}</td>{v.map((val,i)=><td key={i} style={{...S.td,color:val!=="No"&&i===1?"#15803d":"#374151",fontWeight:val!=="No"&&i===1?700:400}}>{val}</td>)}</tr>
          ))}
        </tbody>
      </table>
    </div>
  ...
  if(tab==="reports") {
    const monthly=["Jan","Feb","Mar","Apr","May","Jun"].map((m,i)=>({month:m,leaves:[124,110,115,98,105,120][i]}));
    return (
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <div style={S.card}>
        ...
        <div style={S.card}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            <ResponsiveContainer width="55%" height={180}>
              <PieChart><Pie data={ltData} cx="50%" cy="50%" innerRadius={45} outerRadius={72} dataKey="value">{ltData.map((e,i)=><Cell key={i} fill={e.color}/>)}</Pie><Tooltip formatter={v=>`${v}%`}/></PieChart>
            </ResponsiveContainer>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {ltData.map(d=><div key={d.name} style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:10,height:10,borderRadius:"50%",background:d.color}}/><span style={{fontSize:12,color:"#374151"}}>{d.name}</span><span style={{fontSize:12,fontWeight:700,color:"#0f172a",marginLeft:4}}>{d.value}%</span></div>)}
            </div>
          </div>
        </div>
      </div>
...
function AdminView({tab,holidays,onAddHoliday,onDeleteHoliday,showNotification}) {
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        {holidays.map((h,i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:14,border:"1px solid #e2e8f0",borderRadius:10}}>
            <div><p style={{fontWeight:800,color:"#0f172a",margin:0}}>{h.name}</p><p style={{fontSize:12,color:"#64748b",margin:"3px 0 0"}}>{fmtFull(h.date)}</p></div>
  ...
  if(tab==="settings") return (
    <div style={S.card}>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {[["Absconding Alert Threshold","Days before alert is triggered","5 Days"],["Overlapping Leave Warning","Alert when 2+ members take same dates","Enabled"],["Leave Approval Flow","Number of approval levels required","Single Level"],["Holiday Calendar Region","State-specific holidays applied","Karnataka"]].map(([l,d,v])=>(
          <div key={l} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:14,border:"1px solid #e2e8f0",borderRadius:10}}>
            <div><p style={{fontWeight:700,color:"#0f172a",margin:0,fontSize:14}}>{l}</p><p style={{fontSize:12,color:"#64748b",margin:"3px 0 0"}}>{d}</p></div>
            <span style={{fontSize:14,fontWeight:700,color:"#2563eb"}}>{v}</span>
          </div>
        ))}
...
function ReportsView({tab,requests}) {
  const monthly=[{m:"Jan",leaves:124,rate:8.4},{m:"Feb",leaves:110,rate:7.2},{m:"Mar",leaves:115,rate:7.8},{m:"Apr",leaves:98,rate:6.5},{m:"May",leaves:105,rate:7.0},{m:"Jun",leaves:120,rate:8.1}];
  ...
  if(tab==="dashboard") return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        ...
        <div style={S.card}>
          <h3 style={{fontSize:14,fontWeight:800,color:"#0f172a",margin:"0 0 12px"}}>Quick Stats</h3>
          {[["Total Leave Requests",requests.length,"#2563eb"],["Approved Leaves",requests.filter(r=>r.status==="approved").length,"#16a34a"],["Pending Requests",requests.filter(r=>r.status==="pending").length,"#d97706"],["Absenteeism Rate","8.2%","#dc2626"]].map(([l,v,c])=>(
  ...
  if(tab==="leave-analysis") return (
    <div style={S.card}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:24}}>
        ...
        <div>
          <h4 style={{fontSize:13,fontWeight:700,color:"#64748b",margin:"0 0 12px"}}>2027 Forecast Summary</h4>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:14}}>
            <div style={{padding:14,background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:10,textAlign:"center"}}><p style={{fontSize:22,fontWeight:900,color:"#1d4ed8",margin:0}}>325</p><p style={{fontSize:11,color:"#374151",margin:"3px 0 0"}}>Est. Days</p><p style={{fontSize:10,color:"#16a34a",margin:"2px 0 0",fontWeight:700}}>↑ 5% YoY</p></div>
            <div style={{padding:14,background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:10,textAlign:"center"}}><p style={{fontSize:14,fontWeight:900,color:"#15803d",margin:0}}>Jul–Aug</p><p style={{fontSize:11,color:"#374151",margin:"3px 0 0"}}>Peak Period</p><p style={{fontSize:10,color:"#64748b",margin:"2px 0 0"}}>72 Projected</p></div>
            <div style={{padding:14,background:"#fff7ed",border:"1px solid #fed7aa",borderRadius:10,textAlign:"center"}}><p style={{fontSize:13,fontWeight:900,color:"#c2410c",margin:0}}>⚠️ High</p><p style={{fontSize:11,color:"#374151",margin:"3px 0 0"}}>Overlap Risk</p><p style={{fontSize:10,color:"#64748b",margin:"2px 0 0"}}>Plan Staffing</p></div>
          </div>
        </div>
      </div>
    </div>
...
function App() {
  ...
  const safeGet=(key,def)=>{try{const v=localStorage.getItem(key);return v?JSON.parse(v):def;}catch(e){return def;}};
  const safeSet=(key,val)=>{try{localStorage.setItem(key,JSON.stringify(val));}catch(e){}};

  useEffect(()=>{
    setRequests(safeGet("lms_requests",INITIAL_REQUESTS));
    setHolidays(safeGet("lms_holidays",INITIAL_HOLIDAYS));
    const u=safeGet("lms_user",null);
    if(u)setUser(u);
    setLoading(false);
  },[]);

  const showToast=(msg,type="success")=>{setToast({msg,type});setTimeout(()=>setToast(null),3000);};
  const login=(u)=>{setUser(u);setTab("dashboard");safeSet("lms_user",u);};
  const logout=()=>{setUser(null);try{localStorage.removeItem("lms_user");}catch(e){}};
  ...
  const submitLeave=()=>{
    ...
    const req={id:Date.now(),empId:user.id,empName:user.name,dept:user.dept||"Engineering",type:newLeave.type,start:newLeave.start,end:newLeave.end,days,status:"pending",reason:newLeave.reason,applied:new Date().toISOString().split("T")[0]};
    const updated=[...requests,req];
    setRequests(updated);safeSet("lms_requests",updated);
  ...
  const addHoliday=()=>{
    ...
    setHolidays(updated);safeSet("lms_holidays",updated);
  ...
  if(loading) return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#f1f5f9"}}><div style={{textAlign:"center"}}><div style={{fontSize:48,marginBottom:12}}>⏳</div><p style={{color:"#64748b",fontSize:16,fontWeight:600}}>Loading...</p></div></div>;
  ...
  return (
    <div style={{minHeight:"100vh",background:"#f1f5f9",fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      {toast&&<div style={{position:"fixed",top:16,right:16,zIndex:1000,padding:"12px 24px",borderRadius:10,background:toast.type==="error"?"#dc2626":"#16a34a",color:"#fff",fontWeight:700,fontSize:14,boxShadow:"0 8px 24px rgba(0,0,0,0.2)"}}>{toast.msg}</div>}
      <div style={{background:"#fff",borderBottom:"1px solid #e2e8f0",boxShadow:"0 1px 4px rgba(0,0,0,0.06)",position:"sticky",top:0,zIndex:100}}>
        <div style={{padding:"12px 24px",display:"flex",alignItems:"center",justifyContent:"space-between",maxWidth:1200,margin:"0 auto"}}>
          <div><p style={{fontSize:11,color:"#64748b",fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase",margin:0}}>Leave Management System</p><p style={{fontSize:16,fontWeight:800,color:"#0f172a",margin:"2px 0 0"}}>{DASH_TITLE[user.role]}</p></div>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            <div style={{textAlign:"right"}}><p style={{fontSize:13,fontWeight:700,color:"#0f172a",margin:0}}>Welcome, {user.name}</p><p style={{fontSize:11,color:"#64748b",margin:"2px 0 0",textTransform:"capitalize"}}>{user.role} • {user.dept}</p></div>
        ...
        <div style={{padding:"0 24px",maxWidth:1200,margin:"0 auto",display:"flex",gap:4,overflowX:"auto",borderTop:"1px solid #f1f5f9"}}>
          {tabs.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"10px 16px",fontSize:13,fontWeight:700,whiteSpace:"nowrap",border:"none",borderBottom:tab===t.id?"2.5px solid #2563eb":"2.5px solid transparent",color:tab===t.id?"#2563eb":"#64748b",background:"transparent",cursor:"pointer",fontFamily:"inherit"}}>{t.label}</button>)}
        </div>
      </div>
      ...
      {modal==="apply-leave"&&<Modal title="Apply for Leave" onClose={()=>setModal(null)}>
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          <div><label style={S.label}>Leave Type</label><select value={newLeave.type} onChange={e=>setNewLeave({...newLeave,type:e.target.value})} style={S.input}>{LEAVE_TYPES.map(lt=><option key={lt.id} value={lt.id}>{lt.name} ({lt.total} {lt.unit})</option>)}</select></div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}><div><label style={S.label}>Start Date</label><input type="date" value={newLeave.start} onChange={e=>setNewLeave({...newLeave,start:e.target.value})} style={S.input}/></div><div><label style={S.label}>End Date</label><input type="date" value={newLeave.end} onChange={e=>setNewLeave({...newLeave,end:e.target.value})} style={S.input}/></div></div>
          ...
          {newLeave.start&&newLeave.end&&new Date(newLeave.end)>=new Date(newLeave.start)&&<div style={{padding:"10px 14px",background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:8}}><p style={{fontSize:13,color:"#1d4ed8",fontWeight:700,margin:0}}>📋 {Math.ceil((new Date(newLeave.end)-new Date(newLeave.start))/86400000)+1} days of leave requested</p></div>}
          <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:4}}><button onClick={()=>setModal(null)} style={S.btnGray}>Cancel</button><button onClick={submitLeave} style={S.btnBlue}>Submit Leave Request</button></div>
        </div>
      </Modal>}
      {modal==="add-holiday"&&<Modal title="Add New Holiday" onClose={()=>setModal(null)}>
        <div style={{display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div><label style={S.label}>Type</label><select value={newHoliday.type} onChange={e=>setNewHoliday({...newHoliday,type:e.target.value})} style={S.input}><option value="national">National Holiday</option><option value="state">State Holiday</option><option value="optional">Optional Holiday</option></select></div>
          <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:4}}><button onClick={()=>setModal(null)} style={S.btnGray}>Cancel</button><button onClick={addHoliday} style={S.btnBlue}>Add Holiday</button></div>
        </div>
      </Modal>}
    </div>
  );
...
ReactDOM.createRoot(document.getElementById('root')).render(<App />);  hr:"Manage holidays, policies & analytics",
  admin:"System configuration & settings",
  reports:"View detailed analytics & reports",
};
